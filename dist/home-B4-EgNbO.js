...
function home () {
  ...
  if (infinityPath && heroSection) {
    ...
    const seg1 = pathLength * 0.3;
    const gap1 = pathLength * 0.35;
    const seg2 = pathLength * 0.2;
    const gap2 = pathLength - seg1 - gap1 - seg2;

    // Two asymmetric red segments — desfasados, no se cruzan en el medio
    gsap.set(infinityPath, {
      stroke: 'var(--base--red)',
      strokeDasharray: `${seg1} ${gap1} ${seg2} ${gap2}`,
      strokeDashoffset: 0,
    });

    const loopTl = gsap.to(infinityPath, {
    ...
    ScrollTrigger.create({
      ...
      onLeave() {
        loopTl.kill();
        // Complete: fill the entire stroke
      ...
      onEnterBack() {
        gsap.set(infinityPath, {
          strokeDasharray: `${seg} ${gap} ${seg} ${gap}`,
          strokeDashoffset: 0,
        });
        loopTl.restart();
        if (nav) {
...
//# sourceMappingURL=home-B4-EgNbO.js.map
