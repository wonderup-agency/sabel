...
function home () {
  ...
  if (infinityPath && heroSection) {
    ...
    // Convergence timeline — scrubbed to scroll, not timed
    ...
    // Vertical line draws
    convergeTl.to(verticalPath, { opacity: 1, duration: 0.1, ease: 'none' }, 1.0);
    ...
    ScrollTrigger.create({
      ...
      onUpdate(self) {
        ...
        if (p > 0.3 && !loopKilled) {
          // Kill loop, start scrubbing convergence
          loopKilled = true;
          loopTl.kill();
        }

        if (loopKilled) {
          // Map scroll progress 0.3–1.0 to convergence 0–1
          const convergeP = Math.max(0, Math.min(1, (p - 0.3) / 0.7));
          convergeTl.progress(convergeP);
        }

        if (p < 0.3 && loopKilled) {
          // Scrolled back above threshold — restart loop
          loopKilled = false;
          convergeTl.progress(0);
...
//# sourceMappingURL=home-co5WuQRH.js.map
