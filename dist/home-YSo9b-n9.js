...
function home () {
  ...
  if (infinityPath && heroSection) {
    ...
    // Dot pulses at center while loop is running (hints at scroll)
    let pulseTl = gsap.timeline({ repeat: -1, yoyo: true });
    pulseTl.fromTo(
      centerDot,
    ...
    function playConvergence() {
      ...
      // 2. Dot burst at center when lines arrive
      tl.to(centerDot, {
        scale: 1.2,
        opacity: 1,
        duration: 0.4,
      ...
      // 3. Vertical starts drawing immediately, dot fades
      tl.to(verticalPath, { opacity: 1, duration: 0.1 }, 1.0);
      tl.to(centerDot, {
        scale: 0,
        opacity: 0,
        duration: 0.5,
    ...
    ScrollTrigger.create({
      ...
      onEnterBack() {
        // Kill everything that might be running
        if (convergeTl) {
          convergeTl.kill();
          convergeTl = null;
        }
...
//# sourceMappingURL=home-YSo9b-n9.js.map
