...
function home () {
  ...
  if (infinityPath && heroSection) {
    ...
    // Convergence + vertical exit timeline
    ...
    // Vertical line draws
    convergeTl.to(verticalPath, { opacity: 1, duration: 0.1 }, 1.1);
    ...
    // When reverse finishes, restart the loop
    ...
    ScrollTrigger.create({
      ...
      pin: true,
      onUpdate(self) {
        // Scroll down past threshold: play convergence
        if (self.direction === 1 && self.progress > 0.35 && !loopKilled) {
          loopKilled = true;
          loopTl.kill();
          convergeTl.timeScale(1).play();
        }
        // Scroll up past threshold: reverse convergence
        if (self.direction === -1 && self.progress < 0.35 && loopKilled) {
...
//# sourceMappingURL=home-B7UaCk6J.js.map
