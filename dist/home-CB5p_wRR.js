...
function home () {
  ...
  if (infinityPath && heroSection) {
    ...
    gsap.set(centerDot, { transformOrigin: 'center center', scale: 0, opacity: 0 });

    // Two vertical paths from center outward (up + down)
    // viewBox is 0 0 670 282, center is ~335, 141
    const vertUp = document.createElementNS('http://www.w3.org/2000/svg', 'path');
    const vertDown = document.createElementNS('http://www.w3.org/2000/svg', 'path');
    vertUp.setAttribute('d', 'M335 141 L335 0');
    vertDown.setAttribute('d', 'M335 141 L335 282')
    ;[vertUp, vertDown].forEach((p) => {
      p.setAttribute('stroke', 'var(--base--red)');
      p.setAttribute('fill', 'none');
      p.setAttribute('filter', 'url(#line-glow)');
      infinityPath.parentNode.appendChild(p);
    });
    const vertUpLen = vertUp.getTotalLength();
    const vertDownLen = vertDown.getTotalLength();
    gsap.set(vertUp, {
      strokeDasharray: vertUpLen,
      strokeDashoffset: vertUpLen,
      opacity: 0,
    });
    gsap.set(vertDown, {
      strokeDasharray: vertDownLen,
      strokeDashoffset: vertDownLen,
      opacity: 0,
    });
    ...
    // Convergence timeline — plays/reverses as a continuous animation
    const convergeTl = gsap.timeline({ paused: true });

    // Lines converge to center from top-left (position 0 = path start = center crossing)
    convergeTl.to(line1, {
      strokeDashoffset: 0,
    ...
    // Dot grows early — already visible before lines vanish
    convergeTl.fromTo(centerDot, {
      scale: 0, opacity: 0,
    }, {
      scale: 0.8, opacity: 1,
    ...
    // Verticals emerge from center (dot) outward — up and down simultaneously
    convergeTl.to([vertUp, vertDown], { opacity: 1, duration: 0.05 }, 1.0);
    convergeTl.to(vertUp, {
      strokeDashoffset: 0,
      duration: 0.8, ease: 'power2.out',
    }, 1.0);
    convergeTl.to(vertDown, {
      strokeDashoffset: 0,
      duration: 0.8, ease: 'power2.out',
    }, 1.0);

    // Dot shrinks as verticals grow through it
    convergeTl.to(centerDot, {
      scale: 0, opacity: 0,
    ...
    convergeTl.eventCallback('onComplete', () => {
      animating = false;
    });
    convergeTl.eventCallback('onReverseComplete', () => {
      animating = false;
      loopKilled = false;
    ...
    ScrollTrigger.create({
      ...
      onUpdate(self) {
        ...
        if (self.direction === 1 && self.progress > 0.35 && !loopKilled) {
          animating = true;
          loopKilled = true;
          loopTl.kill();
          convergeTl.timeScale(1).play();
        }
        if (self.direction === -1 && self.progress < 0.35 && loopKilled) {
          animating = true;
          convergeTl.timeScale(1.5).reverse();
        }
      },
      onLeave() {
        if (!animating) initGlobalLines();
      },
      onEnterBack() {
        if (animating) return
...
//# sourceMappingURL=home-CB5p_wRR.js.map
