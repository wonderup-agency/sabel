...
function home () {
  ...
  if (infinityPath && heroSection) {
    ...
    function playConvergence() {
      // Both segments converge to center (pathLength/2 is the crossing point)
      const centerOffset = -(pathLength / 2 - seg / 2);
      const tl = gsap.timeline();

      // 1. Converge both to center, shrink segments
      tl.to([line1, line2], {
        strokeDashoffset: centerOffset,
        strokeDasharray: `${seg * 0.3} ${pathLength - seg * 0.3}`,
        duration: 1,
        ease: 'power2.inOut',
      });

      // 2. Fade out infinity lines, fade in vertical path
      tl.to([line1, line2], { opacity: 0, duration: 0.3 }, '-=0.2');
      tl.to(verticalPath, { opacity: 1, duration: 0.1 }, '-=0.2');

      // 3. Draw vertical line top to bottom
      tl.to(verticalPath, {
        strokeDashoffset: 0,
        duration: 0.8,
        ease: 'power2.out',
      ...
      // 4. Reveal nav + init global lines
      tl.call(() => {
        if (nav) {
...
//# sourceMappingURL=home-DEsA_ZZK.js.map
