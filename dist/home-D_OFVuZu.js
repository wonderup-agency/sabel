...
function home () {
  ...
  if (infinityPath && heroSection) {
    ...
    gsap.set(centerDot, { transformOrigin: 'center center' });

    // Dot pulses at center during loop
    function createPulse() {
      const tl = gsap.timeline({ repeat: -1, yoyo: true });
      tl.fromTo(
        centerDot,
        { scale: 0.3, opacity: 0.4 },
        { scale: 0.7, opacity: 0.8, duration: 1.2, ease: 'sine.inOut' }
      );
      return tl
    }
    let pulseTl = createPulse();
    ...
    function playConvergence() {
      ...
      // 2. Dot appears and pulses when lines vanish
      tl.to(centerDot, {
        scale: 0.6,
        opacity: 1,
        duration: 0.3,
      ...
      tl.call(() => {
        ...
        pulseTl.fromTo(
          ...
          { scale: 0.8, opacity: 1, duration: 0.8, ease: 'sine.inOut' }
        );
      }, null, 1.2);

      // 3. Vertical starts drawing, dot fades
      tl.to(verticalPath, { opacity: 1, duration: 0.1 }, 1.8);
      tl.call(() => {
        if (pulseTl) pulseTl.kill();
      }, null, 1.8);
      tl.to(centerDot, {
    ...
    function resetToLoop() {
      ...
      if (pulseTl) pulseTl.kill();
      pulseTl = null;
      gsap.killTweensOf([line1, line2, verticalPath, centerDot, nav]);
      ...
      gsap.set(line1, { strokeDashoffset: 0 });
      gsap.set(line2, { strokeDashoffset: -offset2 });
      gsap.set(verticalPath, { opacity: 0, strokeDashoffset: vertLength });
      gsap.set(nav, { yPercent: -100 });
    ...
    ScrollTrigger.create({
      ...
      pin: true,
      onUpdate(self) {
        // Only converge when scrolling DOWN past threshold
        if (self.direction === 1 && self.progress > 0.35 && !loopKilled) {
          loopKilled = true;
          loopTl.kill();
          convergeTl = playConvergence();
        }
        // Reset when scrolling UP past threshold
        if (self.direction === -1 && self.progress < 0.35 && loopKilled) {
          resetToLoop();
        }
      },
...
//# sourceMappingURL=home-D_OFVuZu.js.map
