...
2. **Tooling** (`scripts/`, config files) — build pipeline, dashboard, scaffolding scripts
...
- **Resize**: Optional hook called on `window.resize`
...
```
Local dev → commit → push to GitHub → jsDelivr serves from GitHub
  → Tagged releases (@v1.0.0) for production
...
Interactive terminal UI for project management. See `DASHBOARD.md` for full architecture details.
