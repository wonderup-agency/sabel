...
- **Init**:
  ...
  2. Creates a ScrollTrigger on the track (`start: 'top 80%'` on desktop, `'top 90%'` on mobile ≤991px so the line begins drawing a touch earlier as the track enters; `end: 'bottom 50%'`) and uses a manual ticker-based interpolation (`current += diff * 0.08`) against the ScrollTrigger's progress for smooth drawing. The same start point is passed to the optional `'down'` line cap (`buildLineCap` `triggerStart`) so the cap stays in sync.
