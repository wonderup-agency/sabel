...
| `data-scroll-tabs` | `dot`       | Each `.scroll-tabs_icon` (buttons area only)      | Red dot (grey-2/red toggle)      |
...
- **Init**:
  ...
  4. **Initial state**: Resolves `--base--grey-2` (inactive) and `--base--red` (active) from the component's computed style (with `#4c6280` / `#E10600` fallbacks). First tab is active (dot red via `backgroundColor` + glow, button white, panel visible). All other tabs start inactive (dot grey-2, button inherited color, panel hidden, line empty). `filter` is explicitly cleared to `none` on every dot so any leftover Webflow filter doesn't carry over.
...
- **Resize**: Checks breakpoint on every resize. If crossing the 991px threshold:
  - **Desktop → Mobile**: Kills the ScrollTrigger, clears all GSAP inline styles (`height`, `position`, `sticky`, `opacity`, `visibility`, `filter`, `backgroundColor`, `transform`, `color`) — including any `transform` left on `.scroll-tabs_panels_image` elements — so CSS takes over mobile layout.
