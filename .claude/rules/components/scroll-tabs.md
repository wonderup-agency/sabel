...
- **Resize**: Checks breakpoint on every resize. If crossing the 991px threshold:
  - **Desktop → Mobile**: Kills the ScrollTrigger, clears all GSAP inline styles (`height`, `position`, `sticky`, `opacity`, `visibility`, `filter`, `backgroundColor`, `transform`, `color`) so CSS takes over mobile layout.
